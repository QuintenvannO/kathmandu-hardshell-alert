...
def search_kathmandu_products():
    """
    Zoek productlinks via Kathmandu zoekpagina.

    We gebruiken meerdere zoekopdrachten omdat Kathmandu
    kleurvarianten als afzonderlijke productpagina's kan tonen.
    """
    ...
    for model, search_terms in MODELS.items():
        ...
        for term in search_terms:
            ...
            try:
                ...
                print(f"Zoeken mislukt voor {term}: {exc}")
                continue
            ...
            for link in soup.find_all("a", href=True):
                ...
                if "latok alpine" in combined:
                    products.append(
                        ("Rab Latok Alpine", href)
                    )

                elif "latok mountain" in combined:
                    products.append(
                        ("Rab Latok Mountain", href)
                    )

                elif "triolet" in combined:
                    products.append(
                        ("Patagonia Triolet", href)
                    )
    ...
    for model, url in products:
        unique[(model, url)] = True

    return [
        {"model": model, "url": url}
        for model, url in unique.keys()
    ]


# ============================================================
# PRODUCTPAGINA ANALYSEREN
# ============================================================
...
def parse_product(model, url):
    ...
    # Alles wat zichtbaar op de pagina staat.
    ...
    # Eerst zoeken naar elementen die waarschijnlijk prijs bevatten.
    ...
    for pattern in color_patterns:
        ...
        if match:
            color = normalize_text(match.group(1))
            break
